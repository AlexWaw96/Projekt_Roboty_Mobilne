...
namespace Przeszukiwanie_grafu
{
    class Algorytm1
    {
        ...
        /// <summary>
        /// Funkcja do algorytmu a*
        /// </summary>
        /// <param name="W"> Przyjmuje tylko liste wierzcholkow, bo wie ze trzeba przejsc od 0 do 1 </param>
        ...
        public List<int> Uruchom(wierzcholek[] W)
        {
            ...
            while (!W[1].Odwiedzony)
            {
                ...
                // Teraz sprawdzamy do ktorego elementu z listy otwartych przemieszczamy sie
                int ktory_blizej = -1;
                double aktualna_odl = 100000; //jakas duzo wartosc by mogl sie cofac

                for (i = 0; i < Otwarte.Count; i++) {

                    double odl = PM.Odleglosc(W[Otwarte[i]].xy, koncowy) + W[Otwarte[i]].Dr_do_pkt;
                    if (odl < aktualna_odl)
                    {
                        aktualna_odl = odl;
